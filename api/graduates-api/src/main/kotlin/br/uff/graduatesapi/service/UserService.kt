...
class UserService(
	...
	fun findByEmail(email: String): ResponseResult<PlatformUser> {
		try {
			...
			return ResponseResult.Error(Errors.CANT_RETRIEVE_USER)
		}
	}

	fun findAdminsToSendWorkHistoryEmail(): ResponseResult<List<PlatformUser>> {
		try {
			val user = this.userRepository.findAllByRolesContainsAndSendEmailToAdminOnSaveIs(RoleEnum.ADMIN, true)
	...
	fun findGraduateUsersByWorkHistoryStatusNotUpdated(): ResponseResult<List<PlatformUser>> {
		try {
			val users =
				this.userRepository.findGraduatesWithHistoryStatusNotUpdated()
	...
	fun findProfessorUsersByWorkHistoryStatusNotUpdated(): ResponseResult<List<PlatformUser>> {
		try {
			val users =
				this.userRepository.findProfessorsWithGraduatesWithHistoryStatusNotUpdated()
			return ResponseResult.Success(users)
		} catch (ex: Exception) {
			return ResponseResult.Error(Errors.CANT_RETRIEVE_USERS)
		}
	...
	fun updateUser(user: PlatformUser): ResponseResult<PlatformUser> {
		try {
			val updatedUser = this.userRepository.save(user)
			return ResponseResult.Success(updatedUser)
		} catch (ex: Exception) {
			return ResponseResult.Error(Errors.CANT_UPDATE_USER)
		}
	}

	fun updateUserByDTO(userDTO: RegisterDTO): ResponseResult<PlatformUser> {
		try {
			...
			if (userDTO.user.sendEmailToAdminOnSave != null && userDTO.user.roles.contains(RoleEnum.ADMIN)) {
				currentUser.sendEmailToAdminOnSave = userDTO.user.sendEmailToAdminOnSave
			} else if (!userDTO.user.roles.contains(RoleEnum.ADMIN)) {
				currentUser.sendEmailToAdminOnSave = null
			}
	...
	fun createUser(userDTO: RegisterDTO): ResponseResult<PlatformUser> {
		...
		try {
			...
			val sendEmailToAdminOnSave = if (user.roles.contains(RoleEnum.ADMIN)) user.sendEmailToAdminOnSave else null
